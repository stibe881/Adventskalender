const CONTENT_TYPE_META = {
  text: { label: "Text-Nachricht", icon: "💌" },
  voucher: { label: "Gutschein", icon: "🎟️" },
  qrcode: { label: "QR-Code", icon: "📱" },
  video: { label: "Video", icon: "🎬" },
  audio: { label: "Audio/Song", icon: "🎵" },
  gallery: { label: "Bilder-Galerie", icon: "🖼️" },
  scratchcard: { label: "Rubbellos", icon: "🎰" },
  quiz: { label: "Quiz-Frage", icon: "❓" },
  countdown: { label: "Event-Countdown", icon: "⏳" },
};

const THEME_META = {
  partner: { label: "💕 Partner*in – Romantisch" },
  kid: { label: "🎈 Kind – Verspielt & bunt" },
  parents: { label: "🌲 Eltern – Klassisch & elegant" },
  modern: { label: "✨ Modern – Minimalistisch" },
};
