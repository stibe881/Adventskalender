...
const api = {
  ...
  sendCalendar: (id, email, message) => request("POST", `/admin/calendars/${id}/send`, { email, message }),
  listReceived: () => request("GET", "/admin/received"),
  addReceived: (link) => request("POST", "/admin/received", { link }),
  removeReceived: (token) => request("DELETE", `/admin/received/${encodeURIComponent(token)}`),
  receivedStatus: (token) => request("GET", `/admin/received/status/${encodeURIComponent(token)}`),
